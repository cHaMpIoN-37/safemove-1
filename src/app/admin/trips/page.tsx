...
export default function AdminTripsPage() {
    ...
    const fetchStudents = async () => {
      try {
        ...
        try {
          ...
          console.error('Failed to parse JSON from students API response', e);
          return;
        }
  ...
  const handleAddStudentsToTrip = async () => {
    ...
    try {
      ...
      // Fetch phone numbers of selected students
      const phoneNumbers = await Promise.all(
        selectedStudents.map(async (studentId) => {
          ...
          try {
            ...
            console.error('Failed to parse JSON from student API response', e);
            return null;
          }
          return data?.student?.phone || null;
        })
      );

      // Construct trip link (example: https://yourdomain.com/trip?data=encodedPayload)
      const baseUrl = window.location.origin;
      const encodedPayload = encodeURIComponent(JSON.stringify(payload));
      const tripLink = `${baseUrl}/trip?data=${encodedPayload}`;

      // Send SMS with trip link to each student
      await Promise.all(
        phoneNumbers.map((phone) => {
          if (phone) {
            return fetch('/api/sms/send', {
              ...
              body: JSON.stringify({
                to: phone,
                message: `You have been added to a trip. Access your trip details here: ${tripLink}`,
              }),
            });
          }
      ...
      console.error('Error updating student status or sending SMS:', error);
    }
  };
